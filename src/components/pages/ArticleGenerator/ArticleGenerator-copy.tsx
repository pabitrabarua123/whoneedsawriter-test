...
const ArticleGeneratorCopy: React.FC = () => {
...
.catch(error => {
  ...
  const infoBoxBg = useColorModeValue("bg-slate-100", "bg-[#121b46]");
...
<Flex width="100%" justifyContent="space-between" alignItems="center" mb={4} mt={4} gap={4}>
  <div className="flex flex-col" data-tour="article-mode">
    <div className="flex gap-4 items-center mb-2">
      <Button
        colorScheme="brand"
        onClick={() => setIsGodMode(false)}
        className={`flex items-center gap-2 px-7 py-2.5 rounded-full border-none outline-none font-medium text-base cursor-pointer transition-all duration-200 ${
          !isGodMode 
            ? 'font-bold' 
            : `${infoBoxBg} text-slate-400 hover:bg-slate-600`
        }`}
      >
        Lite Mode
        {!isGodMode && <span className="text-lg ml-2">✓</span>}
      </Button>
      <Button
        colorScheme="brand"
    ...
    <div className={`text-sm text-slate-500 rounded-lg p-3 max-w-md ${infoBoxBg}`}>
      {isGodMode 
        ? "God Mode: Uses deep research, SERP & competitor analysis, and LSI keywords to create comprehensive, ultra HQ articles with citations and infographics."
...
<Flex gap={2} width="100%" height="37px" justifyContent="space-between" alignItems="center" mb={2}>
  ...
  <Text fontSize="sm" color="gray.600">
    ...
    : 
    <>
    {balance.balance_text}: {balance.credits}
    </>
    }
  </Text>
</Flex>
  ...
  <Flex width="100%" justifyContent="flex-end" mb={2}>
    <div data-tour="advanced-settings">
      ...
      />
        ...
        <div className="rounded-md w-full" data-tour="keyword-input">
          <Textarea
...
export default ArticleGeneratorCopy;
